...
class CodedNet(nn.Module):
    ...
    def __init__(self):
        ...
        # load doe_fields_dict aperture angle_list  psf_index
        ...
        self.doe_noise_1 = torch.rand(size = [256]).cuda() * 0.10 + 0.95
    ...
    def forward(self, x, batch_idx, wls_id, training):
        ...
        if training is False and len(self.psf_list) != 0:
            ...
            for i in range(x.shape[0]):
                ...
                # cv2.imwrite('./imgs/' + str(batch_idx) + '_meas.png', np.array(meas_bgr[0].transpose(2, 0).transpose(1, 0).cpu() * 255.0).astype(int))
                # cv2.imwrite('./imgs/' + str(batch_idx) + '_gt.png', np.array(gt_bgr[0].transpose(2, 0).transpose(1, 0).cpu() * 255.0).astype(int))
            ...
            for index,wavelength in enumerate(self.wavelengths_str):
                ...
                for fov in self.fovs_str:
                    ...
                    # spy.save_rgb('./psf_imgs/' + fov + wavelength + '.png', np.array(torch.tile(normalized_psf[0,...].detach().cpu(), (1, 1, 3))))
                    # psf_save = normalized_psf[0,...].detach().cpu() / torch.max(normalized_psf[0,...].detach().cpu())
                    # cv2.imwrite('./psf_imgs/' + fov + wavelength + '.png', np.array(torch.tile(psf_save, (1, 1, 3)) * 255.0).astype(int))
