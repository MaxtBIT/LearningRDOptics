...
class Lensgroup(Endpoint):
    ...
    def trace_to_sensor(self, ray, ignore_invalid=False):
        ...
        """
        # trace rays
        ray_final, valid, p_list = self.trace(ray)
        ...
        if ignore_invalid:
            ...
            if len(p.shape) < 2:
                return p
            # p = torch.reshape(p, (np.prod(p.shape[:-1]), 3)) #20230724
        return p, p_list, ray_final.d
    ...
    def trace(self, ray, stop_ind=None):
        ...
        valid, ray_out, p_list = self._trace(ray_in, stop_ind=stop_ind, record=False)
        
        # in world
        ray_final = self.to_world.transform_ray(ray_out)

        return ray_final, valid, p_list
    ...
    def trace_r(self, ray, stop_ind=None):
        ...
        valid, ray_out, p_list, oss = self._trace(ray_in, stop_ind=stop_ind, record=True)
    ...
    def _sample_ray_render(self, N, wav, sample2, sample3, offset):
        ...
        # trace
        valid, ray, p_list = self._trace(Ray(o, d, wavelength, device=self.device))
        return valid, ray, p_list
        ...
        for i in range(stop_ind+1):
            ...
            if record: # TODO: make it pythonic ...
                for os, v, pp in zip(oss, valid.cpu().detach().numpy(), p.cpu().detach().numpy()):
                    if v:
                        os.append(pp)
            p_list.append(p)  #20230718
            ray.o = p
            ray.d = d
        
        if record:
            return valid, ray, p_list, oss  #20230718
        else:
            return valid, ray, p_list  #20230718
